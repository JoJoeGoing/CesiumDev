define([
    ...
    '../OptionsUtil',
    './MarkerLayer'
], function (defined, createGuid,AssociativeArray,CesiumMath,Resource,Marker, destroyObject, DeveloperError, defaultValue, defineProperties,
     Cartesian3, OptionsUtil,MarkerLayer) {
    'use strict';
    ...
    function Layer(options) {
        ...
        this._childs = new AssociativeArray();

        this._code = defaultValue(options.code,[]);
        for(var i = 0; i < this._code.length; i++){
            this._childs.set(this._code[i],undefined);
        }
    ...
    function buildUrl(viewer,baseUrl, options) {
        ...
        var url = baseUrl + options.layerName + '&outputformat=json&maxFeature=50&' + parseInt(Math.random() * 100000) + '&cql_filter=' + cql_filterStr;
